---
name: "done-story"
description: "Verify a completed backend story against backend task definitions."
compatibility: "HiveSpace backend repo custom command"
---

Run this verification checklist after completing a backend story. Check every applicable backend item and fix failures before marking the story done.
...
- If `tasks/verification.md` exists, read only enough to identify any item with a detail bullet that starts with `User-owned E2E:` and treat it as user-run validation outside this command's executable scope. Do not use `tasks/verification.md` as a task checklist.
- Do not use `tasks/config.md` or `tasks/docs-catalog.md` as task sources for this command.
...
- Treat missing API/event catalog updates as backend contract findings, not as docs/catalog task completion failures.

Verification:
- Run the smallest meaningful build/test command for the changed service.
- Run `.\quality-gate.ps1 -Scope backend:<ServiceName>` for the affected service when coverage applies.
- Review the reported measured line coverage for the affected service.
- If coverage is below 90%, add tests and rerun coverage before treating the story as complete.
...
- Do not execute, fail, or mark complete any task marked `User-owned E2E:`; list it separately as pending or user-confirmed validation.

Report:
- List files created and modified.
- List verification commands and results.
- Include the affected service coverage percentage and whether it met the 90% target.
...
- List backend contract findings such as missing API/event catalog updates separately from backend task completion.
- List `User-owned E2E` tasks separately as pending user validation or user-confirmed complete.
