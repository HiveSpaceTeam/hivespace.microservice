...
public class CompleteGoogleCallbackCommandValidator : AbstractValidator<CompleteGoogleCallbackCommand>
{
    public CompleteGoogleCallbackCommandValidator(IConfiguration configuration)
    {
        RuleFor(c => c.App)
            .Must(AccountSessionValidation.IsBuyerOrSellerApp)
            .WithState(_ => new Error(CommonErrorCode.InvalidArgument, nameof(CompleteGoogleCallbackCommand.App)));
        ...
        RuleFor(c => c.ReturnUrl)
            ...
            .WithState(_ => new Error(IdentityDomainErrorCode.InvalidReturnUrl, nameof(CompleteGoogleCallbackCommand.ReturnUrl)))
