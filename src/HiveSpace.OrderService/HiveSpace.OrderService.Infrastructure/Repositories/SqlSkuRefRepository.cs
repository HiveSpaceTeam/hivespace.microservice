...
public class SqlSkuRefRepository(OrderDbContext context) : ISkuRefRepository
{
    ...
    public Task<SkuRef?> GetByIdAsync(long skuId, long productId, CancellationToken cancellationToken = default)
        => context.SkuRefs.FirstOrDefaultAsync(s => s.Id == skuId && s.ProductId == productId, cancellationToken);
