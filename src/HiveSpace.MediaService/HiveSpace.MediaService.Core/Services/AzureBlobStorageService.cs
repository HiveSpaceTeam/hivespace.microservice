...
public class AzureBlobStorageService : IStorageService
{
    ...
    public AzureBlobStorageService(IConfiguration configuration)
    {
        ...
        if (string.IsNullOrEmpty(connectionString))
        {
             ...
             // Helper for debugging locally where keys might be prefixed
        ...
        if (connectionString == null)
            throw new DomainException(500, MediaDomainErrorCode.StorageConfigurationMissing, "AzureBlobStorageService");
    ...
    public async Task<Stream> DownloadBlobAsync(string containerName, string blobName)
    {
        ...
        {
            throw new DomainException(404, MediaDomainErrorCode.BlobNotFound, "AzureBlobStorageService");
        }
    ...
    public async Task DeleteBlobAsync(string containerName, string blobName)
    {
        var containerClient = _blobServiceClient.GetBlobContainerClient(containerName);
        var blobClient = containerClient.GetBlobClient(blobName);
        // DeleteIfExistsAsync returns true if deleted, false if not exists
    ...
    public async Task ConfigureCorsAsync(string[] allowedOrigins, CancellationToken cancellationToken)
    {
        ...
        // Remove existing to clean up or just ensure we have our rule
