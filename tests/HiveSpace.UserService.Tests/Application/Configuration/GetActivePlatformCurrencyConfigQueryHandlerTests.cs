...
public class GetActivePlatformCurrencyConfigQueryHandlerTests : IClassFixture<UserServiceFixture>
{
    private readonly UserServiceFixture _fixture;

    public GetActivePlatformCurrencyConfigQueryHandlerTests(UserServiceFixture fixture) => _fixture = fixture;
    ...
    public async Task Handle_ReturnsItemsDefaultAndVersion()
    {
        ...
        var handler = new GetActivePlatformCurrencyConfigQueryHandler(
            new SqlPlatformConfigRepository(_fixture.DbContext),
            new SqlPlatformCurrencyRepository(_fixture.DbContext));

        var result = await handler.Handle(new GetActivePlatformCurrencyConfigQuery(), CancellationToken.None);
